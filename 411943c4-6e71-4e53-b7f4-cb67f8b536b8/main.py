"""v2 — Volatility-scaled intraday momentum, with ORB as an optional confirmation.

Successor to strategy_development/spy_orb/v1/strategy_v1.py.
...
# Risk
RISK_PCT = 0.015        # target account risk per trade at 1R
...
class TradingStrategy(Strategy):
    ...
    def _evaluate(self, todays, vol_ref, trigger):
        ...
        stop_dist = STOP_MULT * vol_ref          # fraction of price
    ...
    def run(self, data):
        ...
        # Volatility gate. Logged at OR completion above; enforced here.
        if not vol_ok:
            if latest_ts == or_complete_ts:
        ...
        # ---------------------------------------------------------- logging
        if st["entry_ts"] == latest_ts and st["entry_bar"] is not None:
            b = st["entry_bar"]
            log(
        ...
        if at_flat:
            ...
            # The two counterfactuals that make one backtest answer three
            # questions: is the band better than ORB, and is QQQ better than SPY.
            self._log_counterfactual(
                todays, vol_ref, today, TRADE_UNDERLYING,
                "orb" if TRIGGER == "band" else "band", "[V2-ORB]")
            self._log_alt(ohlcv, today, vol_ref)
        ...
        if at_flat or not st["in_position"]:
            return TargetAllocation(flat)
        if st["direction"] == "long":
    ...
    def _log_counterfactual(self, todays, vol_ref, day, und, trigger, tag):
        """Evaluate a rule variant that is NOT traded, and log its outcome.

        Costs nothing statistically and is what lets one backtest answer
        several questions instead of one. Never affects the allocation.
        """
        st = self._evaluate(todays, vol_ref, trigger)
        if st is None:
            return
        vol_ok = int(vol_ref >= VOL_FLOOR)
        if st["entry_ts"] is None:
            log("{tag} {d} UND={u} TRIG={tg} RESULT=none GATE={g} "
                "VOL_REF={v:.5f} VOL_OK={ok}".format(
                    tag=tag, d=day, u=und, tg=trigger, g=st["blocked_by"],
                    v=vol_ref, ok=vol_ok))
            return
        ...
        if st["direction"] == "long":
            ...
            pnl = (st["entry_px"] - exit_px) / st["entry_px"]
        log(
            "{tag} {d} UND={u} TRIG={tg} RESULT={r} DIR={dir} ENTRY_TS={ets} "
            "ENTRY_PX={ep:.2f} EXIT_PX={xp:.2f} PNL_UND={p:.5f} "
            "PNL_ACCT={pa:.5f} ALLOC={a:.4f} VOL_REF={v:.5f} "
            "VOL_OK={ok}".format(
                tag=tag, d=day, u=und, tg=trigger,
                r="stop" if st["stopped"] else "flat",
                dir=st["direction"].upper(), ets=st["entry_ts"],
                ep=st["entry_px"], xp=exit_px, p=pnl,
                pa=pnl * LEV_EFF * st["alloc"], a=st["alloc"],
                v=vol_ref, ok=vol_ok,
            )
        )

    def _log_alt(self, ohlcv, today, _unused):
        """Same rule, other underlying — answers SPY-vs-QQQ from one backtest.

        Uses the ALT underlying's OWN volatility for its bands and sizing, so
        this is a like-for-like vehicle comparison rather than QQQ's parameters
        imposed on SPY.
        """
        alt = self._series(ohlcv, ALT_UNDERLYING)
        if not alt:
            return
        sess = self._sessions(alt)
        todays = sess.get(today, [])
        if len(todays) < OR_BARS:
            return
        vol_ref, _n = self._vol_ref(sess, today)
        if vol_ref is None or vol_ref <= 0:
            return
        self._log_counterfactual(todays, vol_ref, today, ALT_UNDERLYING,
                                 TRIGGER, "[V2-ALT]")